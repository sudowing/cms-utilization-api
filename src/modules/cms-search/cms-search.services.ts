...
const genGeoProviderQuery = (
    ...
    if (geoOptions.top_left && geoOptions.bottom_right) {
        ...
        geoFilter = {
            geo_distance: {
                distance: `${geoOptions.distanceValue}${
                    geoOptions.distanceUnit
                }`,
                location: {
...
export const searchGeoProviders = async (
    ...
    const records: ts.ProviderPerformanceRecord[] = searchResults.hits.hits.map(
        (record: any) => {
            const source = record._source;
            source.performances = source.performances.reduce(
                (
                    accum: ts.ProviderPerformanceDetail[],
                    curr: any,
...
export const searchGeoServiceStats = async (
    ...
    const providerPerformanceRecords = providerPerformanceRecordsIndexDocs.reduce(
        (
            outerAccum: ts.ProviderPerformanceDetail[],
            curr: any,
        ): ts.ProviderPerformanceDetail[] => {
            const providerPerformances: ts.ProviderPerformanceRecord[] =
                curr._source.performances;
            return providerPerformances.reduce(
                (
                    innerAccum: ts.ProviderPerformanceDetail[],
                    item: any,
                ): ts.ProviderPerformanceDetail[] => {
                    // if they search using hcpcs, only return relevant performance records
                    if (hcpcsOptions && hcpcsOptions.hcpcsCodes) {
    ...
    const services = new Set(
        providerPerformanceRecords.map(
            (item: ts.ProviderPerformanceDetail) => item.hcpcs_code,
        ),
    );
    ...
    for (const providerPerformance of providerPerformanceRecords) {
        ...
        aggregateServiceStats[code].n_of_svcs =
            aggregateServiceStats[code].n_of_svcs +
            providerPerformance.n_of_svcs;

        aggregateServiceStats[code].est_ttl_submitted_charge_amt =
            aggregateServiceStats[code].est_ttl_submitted_charge_amt +
            providerPerformance.est_ttl_submitted_charge_amt;
        aggregateServiceStats[code].est_ttl_mcare_pay_amt =
            aggregateServiceStats[code].est_ttl_mcare_pay_amt +
            providerPerformance.est_ttl_mcare_pay_amt;
        aggregateServiceStats[code].est_ttl_mcare_allowed_amt =
            aggregateServiceStats[code].est_ttl_mcare_allowed_amt +
            providerPerformance.avg_mcare_allowed_amt *
                providerPerformance.n_of_svcs;
        aggregateServiceStats[code].est_ttl_mcare_standardized_amt =
            aggregateServiceStats[code].est_ttl_mcare_standardized_amt +
            providerPerformance.avg_mcare_standardized_amt *
                providerPerformance.n_of_svcs;
    }

    const serviceStats = Object.keys(aggregateServiceStats).map(
        (hcpcs_code: string) => {
            ...
            nServiceStats.avg_submitted_charge_amt =
                nServiceStats.est_ttl_submitted_charge_amt / n_of_svcs;
            nServiceStats.avg_mcare_pay_amt =
                nServiceStats.est_ttl_mcare_pay_amt / n_of_svcs;
            nServiceStats.avg_mcare_allowed_amt =
                nServiceStats.est_ttl_mcare_allowed_amt / n_of_svcs;
            nServiceStats.avg_mcare_standardized_amt =
                nServiceStats.est_ttl_mcare_standardized_amt / n_of_svcs;
...
export const suggestProviders = async (
    ...
    const records = searchResults.suggest.hcpcs_suggest[0].options.map(
        (record: any) => {
