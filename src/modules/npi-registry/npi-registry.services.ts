...
export const getProvider = async (npi: number): Promise<RegistryProfile|null> => {
    ...
    const output = (status === 200 && data.result_count === 1) ? data.results[0] as RegistryProfile : null;
    return output;
}